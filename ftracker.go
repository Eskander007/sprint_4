...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	return float64(runningCaloriesMeanSpeedMultiplier) * meanSpeed(action, duration) * float64(runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	return (walkingCaloriesWeightMultiplier*weight + (math.Pow(meanSpeedMInsec, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
